...
import { Github, MessageSquare, Download, AlertCircle, CheckCircle2, RefreshCw, ExternalLink, X, Bot } from "lucide-react";
...
export default function TopRightControls({ onSitrepToggle }: { onSitrepToggle?: () => void }) {
    ...
    return (
        <div className="relative flex items-center gap-2 mb-1 justify-end">
            ...
            {/* AI SITREP toggle button */}
            {onSitrepToggle && (
                <button
                    onClick={onSitrepToggle}
                    className="flex items-center gap-1.5 px-2.5 py-1.5 bg-[var(--bg-primary)]/50 backdrop-blur-md border border-[var(--border-primary)] rounded-lg hover:border-cyan-500/50 hover:bg-[var(--hover-accent)] transition-all text-[10px] text-[var(--text-secondary)] font-mono cursor-pointer"
                >
                    <Bot size={12} className="text-cyan-400 w-3 h-3" />
                    <span className="tracking-widest">SITREP</span>
                </button>
            )}
